...
export const Programs: React.FC = () => {
  ...
  return (
    <div className="flex flex-col w-full bg-[#F8F9FA]">
      ...
      {/* Category Filter Bar */}
      <section className="w-full bg-white border-b border-[#E1E3E4] py-4 sticky top-16.25 z-30 shadow-xs">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col gap-16">
          {filteredPrograms.map((program) => (
            <div
              key={program.id}
              id={program.id}
              className="bg-white rounded-3xl border border-[#E1E3E4] shadow-sm overflow-hidden scroll-mt-28 grid grid-cols-1 lg:grid-cols-12 gap-0"
            >
              {/* Image Column */}
              <div className="lg:col-span-5 relative min-h-75 lg:min-h-full bg-slate-100">
                <img
              ...
              <div className="lg:col-span-7 p-6 sm:p-10 flex flex-col justify-between gap-6 text-left">
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-2xl sm:text-3xl font-extrabold text-[#001E40]">{program.name}</h2>
                      <span className="text-sm font-semibold text-[#006E25]">{program.hindiName}</span>
                    </div>
                  </div>

                  <p className="text-sm sm:text-base text-[#43474F] mt-4 leading-relaxed">
                    {program.fullDescription}
                  </p>

                  {/* Impact Stats Strip */}
                  <div className="grid grid-cols-3 gap-3 mt-6 p-4 bg-[#F8F9FA] rounded-2xl border border-[#E1E3E4] text-center">
                    {program.impactMetrics.map((m, idx) => (
                      <div key={idx} className="flex flex-col">
                        <span className="text-lg font-extrabold text-[#003366]">{m.value}</span>
                        <span className="text-xs text-[#737780] font-medium">{m.label}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  {/* Key Activities List */}
                  <div className="mt-6">
                    <h3 className="text-xs font-bold uppercase tracking-wider text-[#003366] mb-3">Key Ground Interventions</h3>
                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-[#43474F]">
                ...
                {/* Actions */}
                <div className="pt-6 border-t border-[#E1E3E4] flex flex-wrap items-center gap-4">
                  <NavLink to="/donate">
